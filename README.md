...
- **No. Fields**: total fields found in the workbook (and how many are unused)
- **No. Calculated Fields**: total calculated fields, including LODs if any
- **Avg. Upstream Sources**: average number of input fields and/or preceding calculated fields used per calculation
- **No. Sheets**: number of worksheets connected to data fields or parameters
...
- **Item**: field, calculated field, parameter, or sheet
- **Layout**: orientation of the dependency graph

Once selected, the app renders an **interactive dependency graph** built from the extracted Tableau metadata. Each node represents a field or calculation, and arrows indicate dependencies between them.

- Click on any node to view **detailed information** (type, category, role, upstream/downstream levels).
- Follow **upstream sources** or **downstream consumers** interactively.
